...
void visualize_final_path(Node4D &current_node, std::map<int, Node4D> &closed_list) {
	...
	pure_pursuit();			// CHANGE MADE ON 10th AUG UNCOMMENT AFTER DONE
...
bool hybrid_astar_plan() {
	...
	if(path_found){
		...
		listener.waitForTransform("/map", "/tb3_0/base_footprint", ros::Time::now(), ros::Duration(2.0));
		listener.lookupTransform("/map", "/tb3_0/base_footprint", ros::Time(0), transform_robot);
		...
		listener.lookupTransform("/map", "/tb3_0/base_footprint", ros::Time(0), transform_trailer);
...
cmd2.linear.x = 0;
	...
	cout<<goal_final.pose.position.x<<" "<<goal_final.pose.position.y<<endl;
...
void callback_goal_pose(const geometry_msgs::PoseStamped::ConstPtr& pose) {
	...
	gyaw = tf::getYaw(goal_pose.pose.orientation);
if(check4==0)
	{	goal_final.header.stamp = ros::Time::now();
	goal_final.header.frame_id = "map";
	goal_final.pose.position = pose->pose.position;
	goal_final.pose.orientation = pose->pose.orientation;
	check4++;
	}
	check4++;
	goal_pose_pub.publish(goal_pose); // UNCOMMENT (COMMENTED ON AUG 17)
...
int main(int argc, char **argv) {
	...
	cmd_pub = nh.advertise<geometry_msgs::Twist>("/tb3_0/cmd_vel", 1000);
	target_point_pub = nh.advertise<geometry_msgs::PointStamped>("target_point", 1000);
	global_goal = nh.advertise<geometry_msgs::PoseStamped>("tb3_0/move_base_simple/goal", 1000);
	temp_goal_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("tb3_0/initialpose", 1000);
//publishers2
/*
	start_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("start_pose", 1000);
	goal_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("goal_pose", 1000);
...
*/
	...
	// Subscribers
	ros::Subscriber start_pose_sub = nh.subscribe("/tb3_0/initialpose", 1000, callback_start_pose);
	ros::Subscriber goal_pose_sub = nh.subscribe("/tb3_0/move_base_simple/goal", 1000, callback_goal_pose);
