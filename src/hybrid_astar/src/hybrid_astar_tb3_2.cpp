...
#include "../include/hybrid_astar/pure_pursuit_tb3_2.h"
using namespace std;
...
void visualize_final_path(Node4D &current_node, std::map<int, Node4D> &closed_list) {
	...
	pure_pursuit_tb3_2();			// CHANGE MADE ON 10th AUG UNCOMMENT AFTER DONE
...
bool hybrid_astar_plan() {
	...
	if(path_found){
		...
		listener.waitForTransform("/map", "/tb3_2/base_footprint", ros::Time::now(), ros::Duration(2.0));
		listener.lookupTransform("/map", "/tb3_2/base_footprint", ros::Time(0), transform_robot);
		...
		listener.lookupTransform("/map", "/tb3_2/base_footprint", ros::Time(0), transform_trailer);
...
int main(int argc, char **argv) {
	...
	ros::init(argc, argv, "hybrid_astar_tb3_2");

	ros::NodeHandle nh;

	// Publishers
	start_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("tb3_2/start_pose", 1000);
	goal_pose_pub = nh.advertise<geometry_msgs::PoseStamped>("tb3_2/goal_pose", 1000);
	hybrid_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/hybrid_astar_path", 1000);
	hybrid_tractor_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/hybrid_astar_tractor_path", 1000);
	hybrid_trailer_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/hybrid_astar_trailer_path", 1000);
	dubins_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/dubins_path", 1000);
	reeds_shepp_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/reeds_shepp_path", 1000);
	visualize_nodes_pub = nh.advertise<visualization_msgs::Marker>("tb3_2/nodes", 1000);
	robot_polygon_pub = nh.advertise<geometry_msgs::PolygonStamped>("tb3_2/robot_polygon", 1000);
	robot_polyogn_array_pub = nh.advertise<jsk_recognition_msgs::PolygonArray>("tb3_2/robot_polygon_array", 1000);
	trailer_polygon_pub = nh.advertise<geometry_msgs::PolygonStamped>("tb3_2/trailer_polygon", 1000);
	trailer_polyogn_array_pub = nh.advertise<jsk_recognition_msgs::PolygonArray>("tb3_2/trailer_polygon_array", 1000);
	robot_center_pub = nh.advertise<geometry_msgs::PointStamped>("tb3_2/robot_center", 1000);
	trailer_center_pub = nh.advertise<geometry_msgs::PointStamped>("tb3_2/trailer_center", 1000);
	robot_collision_check_pub = nh.advertise<visualization_msgs::Marker>("tb3_2/robot_collision_check", 1000);
	trailer_collision_check_pub = nh.advertise<visualization_msgs::Marker>("tb3_2/trailer_collision_check", 1000);
	voronoi_path_pub = nh.advertise<visualization_msgs::Marker>("tb3_2/voronoi_path", 1000);
	voronoi_sub_goals_pub = nh.advertise<visualization_msgs::MarkerArray>("tb3_2/voronoi_sub_goals", 1000);
	astar_path_pub = nh.advertise<nav_msgs::Path>("tb3_2/astar_path", 1000);
	cmd_pub = nh.advertise<geometry_msgs::Twist>("/tb3_2/cmd_vel", 1000);
	target_point_pub = nh.advertise<geometry_msgs::PointStamped>("target_point", 1000);
	global_goal = nh.advertise<geometry_msgs::PoseStamped>("tb3_2/move_base_simple/goal", 1000);
	temp_goal_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("tb3_2/initialpose", 1000);
//publishers2
/*
...
*/
	...
	// Subscribers
	ros::Subscriber start_pose_sub = nh.subscribe("/tb3_2/initialpose", 1000, callback_start_pose);
	ros::Subscriber goal_pose_sub = nh.subscribe("/tb3_2/move_base_simple/goal", 1000, callback_goal_pose);
