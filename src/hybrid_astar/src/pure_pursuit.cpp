...
*/
/*bool obstacle_decider(float x_r, float y_r, float yaw_r, float x_obs, float y_obs, float yaw_obs)
{
	//****************************************************
float v_obs_x;
float v_obs_y;
...
//*************************************************************************************************
	...
	//ros::spinOnce();	
	v_obs_x=0.25*cos(syaw);
	v_obs_y=0.25*sin(syaw);
	float distance = sqrt((x_obs-x_r)*(x_obs-x_r) + (y_obs-y_r)*(y_obs-y_r));
	
	cout<<"distance: "<< distance<<endl;
	//cout<<"velocity: "<<v<<endl;

	v_rel_x = v_obs_x-0.25*cos(yaw_obs);
	v_rel_y = v_obs_y-0.25*sin(yaw_obs);
	...
	inter=y_obs-slope*x_obs;
	inter1=inter+2/cos(atan2(v_obs_y,v_obs_x));
	inter2=inter-2/cos(atan2(v_obs_y,v_obs_x));
	cout<<"slope"<<slope<<endl;
	distance1 = abs(y_r-x_r*slope-inter1);
	distance2 = abs(y_r-x_r*slope-inter2);
...
void pure_pursuit() {
	...
	listener_robot.waitForTransform("/map", "/tb3_0/base_link", ros::Time::now(), ros::Duration(1.0));

	/*tf::TransformListener listener_trailer;
	tf::StampedTransform transform_trailer;
	listener_trailer.waitForTransform("/map", "/tb3_0/base_link", ros::Time::now(), ros::Duration(2.0));*/

	/*tf::TransformListener listener_obs_1;
	tf::StampedTransform transform_obs_1;
	listener_obs_1.waitForTransform("/map", "/tb3_1/base_link", ros::Time::now(), ros::Duration(1.0));
...
//ros::Duration(0.05).sleep();
	...
	while(true) {
		...
		listener_robot.lookupTransform("/map", "/tb3_0/base_link", ros::Time(0), transform_robot);
		//listener_trailer.lookupTransform("/map", "/tb3_0/base_link", ros::Time(0), transform_trailer);
...
listener_obs_2.lookupTransform("/map", "/tb3_2/base_link", ros::Time(0), transform_obs_2);
...
//cout<<obstacle_decider(x_r,y_r,yaw_r,x_obs_1,y_obs_1,yaw_obs_1)<<endl;
//cout<<obstacle_decider(x_r,y_r,yaw_r,x_obs_2,y_obs_2,yaw_obs_2)<<endl;
			float check=0;
			...
			srv1.request.r_r=yaw_t;
				...
				cout<<"kl"<<endl;
				listener_robot.lookupTransform("/map", "/tb3_0/base_footprint", ros::Time(0), transform_robot);
